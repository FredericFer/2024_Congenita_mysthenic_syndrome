...
Symptome_init_cpt = data.frame(
  ...
  "Respiratory involvement" =cpt_perc2(data_tempo$Respiratory),
  "Sudden childhood respiratory insuffisiency" =cpt_perc2(data_tempo$Episodes.de.DRA.dans.l.enfance),
  ...
  "Delayed Motor Milestone" =cpt_perc2(data_tempo$Delayed.MM),
...
Inital_ht <- Heatmap(as.matrix(Symptome_init)*100,cluster_rows = new_dend,  # The Heatmap function takes a matrix as input, here we convert our data frame (mat) to a matrix
                     ...
                     name = "Proportion of symptoms (%)",  # This parameter provides a name for the heatmap legend
                      ...
                      column_title = "Initial phenotype",column_title_side = "bottom",column_title_gp = gpar(fontsize = 15, fontface = "bold"),
...
list_ht[i] <- Heatmap(as.matrix(Symptome_init)*100,  # The Heatmap function takes a matrix as input, here we convert our data frame (mat) to a matrix
                      ...
                      column_title_gp = gpar( fontface = "bold"),row_order = tempo2,
...
colnames(mat_new) = c("Acetylcholinesterase Inhibitor","3.4-diaminopyridine","Fluoxetine","Quinidine","Ephedrine" ,"Salbutamol","gene")
...
neutral_palette <- c("Improvement" = "green3", "No Effect" = "grey90", "Ongoing" = "#666666", "Worsening"  = "red")
...
#===============================================================================
proportion_data$Treatment <- factor(proportion_data$Treatment, levels = c("Acetylcholinesterase Inhibitor","3.4-diaminopyridine","Salbutamol","Ephedrine" ,"Fluoxetine","Quinidine"))
...
ggplot(proportion_data, aes(x = "", y = prop, fill = Outcome)) +
  ...
  # Personnalisation du thème
  theme(
    strip.text.x = element_text(face = "bold", size = 14, angle = 0),  # Titre des colonnes
    strip.text.y = element_text(face = "bold", size = 14, angle = 0),  # Titre des lignes
    strip.background = element_rect(fill = "white", color = "grey", size = 1),  # Fond des titres des facettes
